...
instead, you must use either the map and
filter functions or list comprehensions.
'''
...
def threes(n):
    ...
    '''
    return list(filter(lambda x: '3' in list(str(x)), range(0, n+1)))


def small_words(text):
    '''
    Returns a list of all words in the input text that
    are less than or equals to 4 characters long.
    ...
    ['a', 'big', 'word', 'is', 'bad']
    '''
    return list(filter(lambda x: len(x) <= 4, text.split()))


def squares(n):
    '''
    Returns a list of all square
